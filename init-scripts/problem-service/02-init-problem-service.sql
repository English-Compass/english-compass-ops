...
-- Session question categories table (ElementCollection 지원)
CREATE TABLE session_question_categories (
    session_question_id BIGINT NOT NULL COMMENT '세션 문제 ID',
    ...
    -- 인덱스
    INDEX idx_session_question_id (session_question_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='세션 문제별 카테고리 테이블';

-- User profile table (이벤트 기반 프로필: UserService에서 이벤트로 전달받는 정보만 저장)
...
-- 문제 답변 복합 인덱스: user_id + is_correct (오답 조회 최적화)
CREATE INDEX idx_question_answer_user_correct ON question_answer(user_id, is_correct);
